...
* Author MhankBarBar × Farhan × SangLord
...
const { help, bahasa, donasi, limitend, limitcount, bottt } = require('./LORD/help')
...
const { vnmenu } = require('./LORD/vnmenu.js')
const { nsfwmenu } = require('./LORD/nsfwmenu.js')
const { premium } = require('./LORD/premium.js')
...
prefix = "!"
name = "~ LORD BOTZ"
rdaftar = "TERIMA KASIH TELAH DAFTAR MENJADI TEMEN LORDBOTZ😁"
rmenu = "HAI TEMEN LORDBOTZ👋 JANGAN LUPA DONASI YAA:)"
...
async function starts() {
	...
	frhan.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: '*⏳ follow dulu ig gua @sanglord_*',
				success: '*ꜱᴜᴋꜱᴇꜱ...*',
				error: {
				...
				only: {
					...
					userB: `Hai Kak ${pushname2} Kamu Belom Terdaftar Didatabase [SangLordBotz] Silahkan Ketik \n${prefix}daftar`,
       ...
       if (messagesLink.includes("://chat.whatsapp.com/")){
		...
		case 'daftar':
					frhan.updatePresence(from, Presence.composing)
					if (isUser) return reply('kamu sudah Menjadi Temen LORDBOTZ:D')
					if (isBanned) return reply(mess.only.benned)
					user.push(sender)
          ...
          client.sendMessage(from, rules(prefix), text, { quoted: mek })
			...
			if (!isPublic) return reply(mess.only.publikG)
				if (!isUser) return reply(mess.only.userB)
					...
					client.sendMessage(from, premium(prefix), text, { quoted: mek })
					break
    			case 'infobot':
         ...
         case 'lirik':
				...
				case 'hentai':
				    ...
				    if (isLimit(sender)) return reply(limitend(pushname2))
						...
						buffer = await getBuffer(res.result)
						frhan.sendMessage(from, buffer, image, {quoted: mek, caption: 'ni anjim follow ig gua @sanglord_'})
					} catch (e) {
						console.log(`Error :`, color(e,'red'))
						reply(' *ERROR* ')
					}
...
//akhir kreator
		    ...
		    case 'map':
				...
				case 'clearall':
					...
					reply(`\`\`\`Sukses delete all chat LORDBOTZ\`\`\``)
					break
                                case 'bcgc':
					frhan.updatePresence(from, Presence.composing) 
					if (!isOwner) return reply(mess.only.ownerB)
				...
				case 'bc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							sendMess(_.jid, `[ *LORDBOTZ BROADCAST* ]\n\n${body.slice(4)}`)
						}
						reply('Suksess broadcast')
					}
					break
				...
				default:
                  ...
                  reply(`Maaf Kak, Command *${prefix}${command}* Tidak Terdaftar Di Dalam Database *${prefix}menu*`)
					...
					if (isGroup && isSimi && budy != undefined) {
						...
						console.log(color('[LORDBOTZ]','aqua'), 'Command Tidak Terdaftar', color(sender.split('@')[0]))
					}
                           }
		} catch (e) {
